...
@section('content')
  ...
  <section class="title-section text-center text-sm-center revealator-slideup revealator-once revealator-delay1">
    <p class="utf-section-description">Single Blog Post</p>
    <h1>Blog Single <span>Post</span></h1>
    ...
    <div class="container">
      <div class="row">
        ...
        <article class="col-12">
          <img src="{{asset('uploads/blog')}}/{{$blog->thumbnail}}" class="img-fluid w-100 rounded" alt="Blog image"/>
          <h1 class="text-uppercase text-capitalize mt-3">{{$blog->title}}</h1>
          <div class="meta">
              <span><i class="fa fa-user"></i> By, {{$blog->rel_to_user->name}}</span>
              <span class="date mx-3"><i class="fa fa-calendar"></i> {{$blog->created_at->format(date('d-m-Y'))}}</span>
              <span><i class="fa fa-tags"></i> {{$blog->technology}}</span>
          </div>
        <div class="utf-blog-excerpt pb-2 my-4">
          ...
          <!--  Comments Starts -->
          <div class="comments">
            <h3 class="utf-comments-heading">{{$blog_comment->count()}} Comments</h3>

            @forelse ($blog_comment as $comment)
            ...
            <div class="comments-form">
              <form method="POST" action="{{route('frontend.blog.comments')}}">
                  @csrf
                  <div class="row">
